...
const processFiles = async (directory) => {
    ...
    try {
        ...
        // 读取所有文件内容
        const allruleData = await Promise.all(ruleFiles.map(file => readFile(`${directory}/${file}`, 'utf8')));
        // 处理文件规则
        ...
        // 读取所有文件内容
        const allallowData = await Promise.all(filess.map(file => readFile(`${directory}/${file}`, 'utf8')));
        // 处理文件规则
